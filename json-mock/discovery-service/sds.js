...
app.get("/services", cors(), (req, res, _next) => {
  if (req.query.group) {
    return res.json(
...
app.get("/metrics/:service/:version/:instance", cors(), (req, res, _next) => {
...
app.get("/threads/:service/:version/:instance", cors(), (req, res, _next) => {
  const { service, version, instance } = req.params;
  const selectedService = services.find(
    (serviceObj) =>
      serviceObj.name === service && serviceObj.version === version
